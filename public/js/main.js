...
function showLogin() { $('login-view').style.display = ''; $('app-view').style.display = 'none'; if (state.pollTimer) clearInterval(state.pollTimer); }
function showApp() {
  ...
  buildNav();
  loadAll();
  state.pollTimer = setInterval(refreshLive, 3000);
}
