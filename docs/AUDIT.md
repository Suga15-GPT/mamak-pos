...
| 41 | **High** | **Session tokens travel in the URL.** `GET /api/stream` accepts `?token=` because `EventSource` cannot set an `Authorization` header, putting a live session token into reverse-proxy access logs and browser history. Accepted deliberately in phase 06; phase 11's cookie auth removes the reason it exists and must delete the fallback. |
| 42 | **High** | **Receipts are not compliant and cannot be made so.** Phase 08's template reads `restaurant_name`, `restaurant_address` and `sst_number` from `settings`, but nothing writes them — so every receipt prints "Mamak POS" with no address and no SST registration number, which an SST-registered business is required to show. Phase 09. |
| 43 | Low | **Flaky test ports.** Five `test/unit/` files each roll their own `20000 + random*30000` port and collide under a full-suite run, producing intermittent `EADDRINUSE` failures unrelated to the code under test. Phase 09. |
