...
async function setup(base) {
  ...
  return {
    adminAuth,
    cardId: cards[0].id, cardId2: cards[1].id, card2Label: `Card ${cards[1].number}`,
...
test('add-on opens a new kitchen round: round 2 is SENT while round 1 stays SERVED, one bill', async () => {
  await withDb(async () => {
    ...
    const created = await fetch(`${base}/api/orders`, {
      ...
      body: JSON.stringify({ card_id: s.cardId, items: [{ item_id: s.roti.id, qty: 1 }, { item_id: s.telur.id, qty: 1 }] }),
    });
    ...
    assert.equal(order.status, 'sent', 'the card reads as a new order again');
...
test('one round spanning two stations makes two tickets and one bill', async () => {
  await withDb(async () => {
    ...
    const { id: orderId } = await json(await fetch(`${base}/api/orders`, {
      method: 'POST', headers: s.adminAuth,
      body: JSON.stringify({ card_id: s.cardId, items: [{ item_id: s.mee.id, qty: 1 }, { item_id: s.teh.id, qty: 1 }] }),
    }));
...
test('a still-sent add-on can be voided by staff even after round 1 was served', async () => {
  await withDb(async () => {
    ...
    const { id: orderId } = await json(await fetch(`${base}/api/orders`, {
      method: 'POST', headers: s.adminAuth,
      body: JSON.stringify({ card_id: s.cardId, items: [{ item_id: s.roti.id, qty: 1 }] }),
    }));
    const t1 = (await tickets(base, s, 'kitchen')).find(t => t.order_id === orderId);
    for (const st of ['preparing', 'ready', 'served']) await advance(base, s, t1.id, st);
...
/* Master spec §50 */
test('moving an order to another card keeps the bill, the rounds and the audit trail', async () => {
  await withDb(async db => {
    ...
    const { id: orderId } = await json(await fetch(`${base}/api/orders`, {
      method: 'POST', headers: s.adminAuth,
      body: JSON.stringify({ card_id: s.cardId, items: [{ item_id: s.roti.id, qty: 2 }] }),
    }));
    const before = await getOrder(base, s, orderId);

    const moved = await fetch(`${base}/api/orders/${orderId}/move`, {
      method: 'POST', headers: s.adminAuth, body: JSON.stringify({ card_id: s.cardId2 }),
    });
    assert.equal(moved.status, 200);

    const after = await getOrder(base, s, orderId);
    assert.equal(after.id, orderId, 'same dining order');
    assert.equal(after.card_id, s.cardId2);
    assert.equal(after.label, s.card2Label);
    ...
    assert.equal(audit.rows[0].detail.to_card_id, s.cardId2);

    // Moving onto a card that already has its own open order is refused.
    await fetch(`${base}/api/orders`, {
      method: 'POST', headers: s.adminAuth,
      body: JSON.stringify({ card_id: s.cardId, items: [{ item_id: s.roti.id, qty: 1 }] }),
    });
    const clash = await fetch(`${base}/api/orders/${orderId}/move`, {
      method: 'POST', headers: s.adminAuth, body: JSON.stringify({ card_id: s.cardId }),
    });
    assert.equal(clash.status, 409);
  });
});

/* Two tablets sending to the same card in the same instant both read the same
...
test('concurrent sends to one card produce distinct rounds, never a lost batch', async () => {
  await withDb(async () => {
    const base = await startApp();
    const s = await setup(base);
