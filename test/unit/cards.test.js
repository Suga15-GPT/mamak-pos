...
test('a combined bill is never part-paid; a card paid part-way before combining blocks un-combining; with no payment un-combining leaves both cards as they were', async () => {
  await withDb(async db => {
    const base = await startApp();
    const s = await setup(base);
    ...
    // Card 2 pays RM1.00 on its own, then is combined with Card 5: that
    // payment stays attached, so the combined bill can't be split apart.
    ...
    assert.equal(g2.amount_due, 10.13);
    ...
    assert.equal((await db.query('SELECT count(*)::int n FROM payments WHERE order_id = ANY($1::int[])', [[o2, o5]])).rows[0].n, 1);

    // Two legs that settle it exactly, allocated lowest card first.
    const full = await json(await post(base, s, `/api/bill-groups/${g2.id}/pay`, { legs: [{ method: 'Card', amount: 5 }, { method: 'Card', amount: 5.13 }] }));
    assert.equal(full.settled, true);
    assert.deepEqual(full.allocations.map(x => [x.card_number, x.amount]), [[2, 1.12], [5, 3.88], [5, 5.13]], 'lowest card first');
